...
from src.preprocessing import get_class_weights, get_value_weights
###
...
def resample(df_input, class_cols, n_samples):
    ...
    def get_weighted_sampled_index(df, class_cols, n_samples):
        ...
        for i in range(1, n_samples+1):
            class_value_probs = get_class_and_value_weights(df2.iloc[df2_ind + batch], class_cols)
            ...
            subset_df = df2[class_cols[col_ind[0]]]
    ...
    batch = get_weighted_sampled_index(df_input, 
                                       class_cols,
                                       n_samples)

    return df_input.iloc[batch].copy()
