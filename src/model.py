...
try:
    from src.preprocessing import scale_range, pipeline
except:
...
class Train_Image_Data(Dataset):
    def __init__(self, X, y, using_preprocessed_images, dim):
        self.X = X
        self.y = y
        self.using_preprocessed_images = using_preprocessed_images
        self.dim = dim
        #
        ...
        #
        with open(f"config/pipeline_kwargs.yaml", "r") as params:
            self.pipeline_kwargs = yaml.safe_load(params) 
    ...
    def __getitem__(self, index):
        ...
        if self.using_preprocessed_images:
            img = decode_image(path).float()
        else:
            imgs, _ = pipeline(path, [self.dim], **self.pipeline_kwargs)
            img = torch.tensor(np.array(imgs[0])).to(torch.float).unsqueeze(0)
...
def get_loss(criterion, pred, actual):
    ...
    for ind,(output, criterion_i) in enumerate(zip(pred, criterion)):
        if ind==0:
            loss = criterion_i(output, actual[:,ind])
        else:
            loss += criterion_i(output, actual[:,ind])
...
def fit(device, model, train_dataloader, valid_dataloader, weights, min_epochs, max_epochs, unfreeze_all_layers_at_epoch, update_learning_rate_at_epoch, freeze_all_but_final_layer, use_weights, ep, n_classes, save_file_name, lr1, lr2, **kwargs):
    # --- Run the model
    ...
    for epoch in range(max_epochs):
        ...
        if (epoch + 1 >= min_epochs):
            avg_valid_loss = validate(device, model, valid_dataloader, n_classes, criterion)
...
def validate(device, model, dataloader, n_classes, criterion, **kwargs):
    print("Validating...")
    model.eval()
    valid_loss_sum = 0
...
def get_predictions(device, model_path, df_path, path_col, predict_type, batch_size, sample_frac, confmat_fig_name, class_cols, save_file_name, model_type, using_preprocessed_images, dim, **model_params):
    ...
    dataloader = DataLoader(Validate_Image_Data(X, y, using_preprocessed_images, dim), 
