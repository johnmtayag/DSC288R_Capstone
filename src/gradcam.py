...
from src.preprocessing import get_hilbert_index, hilbert_ravel, scale_range, pipeline
...
def plot_gradcam_overlays_gridspec(img_np, gradcam_images, softmax_probabilities, target_labels, predicted_labels, save_folder, save_combined_gradcam_fig, fig_suptitle, enhanced, model_type, dim, **kwargs):
    # Prepare plots
    fig = plt.figure(figsize=(7,5))
    gs = fig.add_gridspec(5, 10, hspace=1.25, wspace=0.05,
                          top=0.9, bottom=0.001, left=0.005, right=0.995)
    axes = []
    ...
    if save_combined_gradcam_fig:
        ...
        if enhanced:
            save_path = f"results/gradcam/{save_folder}/ENHANCED_{model_type}_BASE{dim}_allGradcams.png"
        else:
            save_path = f"results/gradcam/{save_folder}/RAW_{model_type}_BASE{dim}_allGradcams.png"
        plt.savefig(save_path)
        print(f"Combined figure saved to {save_path}")
    
    return fig, axes

def save_separate_gradcams(gradcam_images, softmax_probabilities, target_labels, predicted_labels, patientID, model_type, dim, save_folder, enhanced, **kwargs):
    for i,(gradcam_image, softmax_prob, cond, target_label, pred_label) in enumerate(zip(gradcam_images, 
                                                                                         softmax_probabilities,
                                                                                         conditions,
                                                                                         target_labels,
                                                                                         predicted_labels)):
        img = Image.fromarray(gradcam_image)
        
        # Create plot with title text
        probs_str = ", ".join([f"{p:.2f}" for p in softmax_prob[0]])  # Format probabilities
        title_text = (
            f"{cond}\n"
            f"Target: {target_label} | Pred: {pred_label}\n"
        ...
        #
        if enhanced:
            save_path = f"results/gradcam/{save_folder}/{patientID}_ENHANCED_{model_type}_BASE{dim}_gradcam_condition_{i}_{cond}.png"
        else:
            save_path = f"results/gradcam/{save_folder}/{patientID}_RAW_{model_type}_BASE{dim}W_gradcam_condition_{i}_{cond}.png"
        plt.savefig(save_path)
        plt.close()

        # Create plot without title text
        title_text = " \n \n \n"
        ...
        #
        if enhanced:
            save_path = f"results/gradcam/{save_folder}/{patientID}_ENHANCED_{model_type}_BASE{dim}_gradcam_condition_{i}_{cond}_noTitle.png"
        else:
            save_path = f"results/gradcam/{save_folder}/{patientID}_RAW_{model_type}_BASE{dim}W_gradcam_condition_{i}_{cond}_noTitle.png"
        plt.savefig(save_path)
        plt.close()

        print(f"Individual gradcam image saved to {save_path}")
