...
import scripts.model_scripts as model_scripts
...
# --- Select the number of samples (5000, 30000, 120000)
n_samples_list = [120000] * len(enhanced_images_list)

# --- Choose ResNet50 or DenseNet121 or ResNet152
...
raw_train_filepath = f"{root}train_df.csv"
train_filepath = f"{root}train_df_oversampled.csv"
train_filepaths = [f"{root}train_df_oversampled.csv"] * len(enhanced_images_list)
#
valid_batch_size = 64
valid_sample_frac = 1
...
classWeights_filepath = f"{root}classWeights.csv"
valueWeights_filepath = f"{root}valueWeights.csv"
...
# for dim,enhanced_images in zip(dim_list, enhanced_images_list):
    ...
    n_instances = n_samples_i
    
    if dim==224:
        # --- Starting learning rate
        train_params["lr1"] = 0.00075
        # --- Second learning rate
        train_params["lr2"] = 0.00000015
    else: 
        train_params["lr1"] = 0.001
        train_params["lr2"] = 0.00000025
    ...
    if __name__=="__main__":
        ...
        if train_model:
            ...
            classWeights_df = (pd.read_csv(classWeights_filepath)["train_df"] + pd.read_csv(classWeights_filepath)[str(n_samples_i)]) / 2
            classWeights_df = classWeights_df.tolist()
            # valueWeights_df = pd.read_csv(valueWeights_filepath)[str(n_samples_i)].tolist()
            train_params["classWeights_df"] = classWeights_df
            ...
            if use_weights:
                ...
                weights = model_scripts.get_loss_weights(device, class_value_counts, n_instances, n_classes)
            ...
            with open(f"models/{save_file_name}_losses.txt", "w") as file:
                file.write(str(model_losses))
            
        ###
        predict_params["confmat_fig_name"] = f"models/{save_file_name}.png"
        ...
        if validate_model:
            ...
            outputs["df"].to_csv(f"models/{save_file_name}_{predict_type}_withPredictionsAndConfidences.csv", index=False)
